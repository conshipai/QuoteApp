// src/components/shared/AddressBook.jsx
...
const AddressBook = ({ isDarkMode, onSelect, type = 'all' }) => {
  ...
  // 3D) Update filtering to honor types (with backward-compat fallback to .type)
  const filteredCompanies = companies.filter((c) => {
    ...
    const hasTypes = Array.isArray(c.types) && c.types.length > 0;
    const matchesType =
      (hasTypes && c.types.includes(filterType)) ||
      (!hasTypes && c.type === filterType); // fallback for legacy records

    return matchesSearch && matchesType;
  });

  const CompanyForm = ({ company, onSave, onCancel }) => {
    // 3A) Initialize form with types array (migrating old type)
    const [formData, setFormData] = useState(() => {
      const base = company || {};
      const initialTypes =
        Array.isArray(base.types) ? base.types :
        base.type ? [base.type] : ['shipper'];
    ...
    const handleTypeToggle = (type) => {
      const set = new Set(formData.types || []);
    ...
    return (
      <div className={`p-6 rounded-lg ${isDarkMode ? 'bg-gray-800' : 'bg-white'} shadow-lg`}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {/* 3C) Replace old select with checkboxes */}
          <div>
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    ...
                    {company.isDefault && <Star className="w-4 h-4 text-yellow-500 fill-yellow-500" />}
                    {/* Show one badge per type */}
                    {typesArr.map((t) => (
                      <span
                        key={t}
                  ...
                  <div className={`text-sm space-y-1 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                    ...
                    {company.notes && <div className="text-xs italic">{company.notes}</div>}
                  </div>
                </div>
